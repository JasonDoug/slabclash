...
describe('AuthService', () => {
  ...
  describe('signup', () => {
    it('should throw ConflictException if user exists', async () => {
      ...
      await expect(service.signup({ username: 'test', email: 'test@test.com', password: 'password' })).rejects.toThrow(ConflictException);
    });
    ...
    it('should create a user and return it without passwordHash', async () => {
      ...
      const result = await service.signup({ username: 'test', email: 'test@test.com', password: 'password' });
      expect(result).not.toHaveProperty('passwordHash');
      expect(result).toEqual({ id: '1', username: 'test', email: 'test@test.com' });
    });
  });
  ...
  describe('login', () => {
    ...
    it('should throw UnauthorizedException if password mismatch', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({ passwordHash: 'hash' });
      (bcrypt.compare as jest.Mock).mockResolvedValue(false);
      await expect(service.login({ email: 'test@test.com', password: 'password' })).rejects.toThrow(UnauthorizedException);
    });

    it('should return accessToken and safe user if successful', async () => {
      const dbUser = { id: '1', username: 'test', email: 'test@test.com', passwordHash: 'hash' };
